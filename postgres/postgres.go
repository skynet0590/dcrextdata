...
const (
	...
	GetExchangeDataTicks = `SELECT exchange FROM exchange_data`
...
func (pg *PgDb) GetExchangeDataTick(){
	rows, err := pg.db.Query(GetExchangeDataTicks)
	if err != nil {
	}
    ...
    for rows.Next() {
    	...
    	// 	exchange string
    	// )
        // , &low, &open, &close, &time, &exchange
        err = rows.Scan(&high, &exchange)
        if err != nil {
			// log.Fatal(err)
		}
		y = append(y, U{high: high, exchange: exchange})
		// fmt.Println(high, low, open, close, time, exchange)
    }
}
