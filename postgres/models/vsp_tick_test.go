...
func testVSPTickToManyVSPTickTimes(t *testing.T) {
	...
	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, vspTickDBTypes, true, vspTickColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize VSPTick struct: %s", err)
	}
	...
	if err = randomize.Struct(seed, &b, vspTickTimeDBTypes, false, vspTickTimeColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, vspTickTimeDBTypes, false, vspTickTimeColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if v.VSPTickID == b.VSPTickID {
			bFound = true
		}
		if v.VSPTickID == c.VSPTickID {
			cFound = true
		}
	}
...
func testVSPTickToManyAddOpVSPTickTimes(t *testing.T) {
	...
	foreigners := []*VSPTickTime{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, vspTickTimeDBTypes, false, strmangle.SetComplement(vspTickTimePrimaryKeyColumns, vspTickTimeColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*VSPTickTime{
		{&b, &c},
		{&d, &e},
	}
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.ID != first.VSPTickID {
			t.Error("foreign key was wrong value", a.ID, first.VSPTickID)
		}
		if a.ID != second.VSPTickID {
			t.Error("foreign key was wrong value", a.ID, second.VSPTickID)
		}

		if first.R.VSPTick != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.VSPTick != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.VSPTickTimes[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.VSPTickTimes[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}
...
func testVSPTicksUpdate(t *testing.T) {
	...
	if len(vspTickColumns) == len(vspTickPrimaryKeyColumns) {
...
func testVSPTicksSliceUpdateAll(t *testing.T) {
	...
	var fields []string
	if strmangle.StringSliceMatch(vspTickColumns, vspTickPrimaryKeyColumns) {
		fields = vspTickColumns
	} else {
